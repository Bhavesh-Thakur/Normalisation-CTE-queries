...
Select * from film;
-- In film table there is a two values given in the same row of special_features which violates the 1NF. so, we need to create another table 
-- to normalize the film table to represent the relationship between film mand special features.
...
-- Now we can insert values in deffirent rowa to achieve 1NF. 
...
-- Choose a table in Sakila database and describe how you would determine whether it is in 2NF. if it violates 2NF, 
...
-- CTE Basics:
-- Q5. Write a query using a CTE to retrieve the distinct list of actor names and the number of films they have acted in from 
...
-- Recursive CTE:
-- Q6. Use a recursive CTE to grnerate a hierarchical list of categories and their subcategories from the category table in sakila. 
